# session-chatgpt-login
自写的插件，用于在插件中直接输入Session或者ACCESS_TOKEN可以登录chatgpt
ChatGPT Session 登录（Edge 插件）
这是一个本地 Edge 插件，可把你本人账号的 ChatGPT Session 令牌写入 chatgpt.com 的安全 Cookie，然后打开 ChatGPT。

安装
在 Edge 地址栏打开 edge://extensions/。
打开左侧的“开发人员模式”。
点击“加载解压缩的扩展”，选择本项目文件夹。
将插件固定到工具栏，点击图标即可使用。
使用
粘贴原始 Session 令牌、完整的 __Secure-next-auth.session-token=...，或包含 sessionToken 字段的完整 JSON；插件只会提取 Session 令牌，不会使用其中的 accessToken。复制内容中的空白、零宽字符和反斜杠续行符会自动清除。
点击“写入会话并打开 ChatGPT”。
插件写入 Cookie 后会立即清空输入框，并打开 https://chatgpt.com/。
